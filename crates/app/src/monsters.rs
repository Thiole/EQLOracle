...
/// logged nearby".
fn counts_as_pull(ing: &Ingest, e: &eqlp_store::Encounter, you: Sym) -> bool {
...
/// a table it wouldn't use. Only scans `Store::encounters` (a few
/// thousand entries even on a long-lived character), never the millions
/// of individual events under them.
pub fn mob_stats(ing: &Ingest, name: &str) -> MobStatsDto {
    ...
    for e in &ing.store.encounters {
        if !ing.store.name(e.target).eq_ignore_ascii_case(name) || !counts_as_pull(ing, e, you) {
            continue;
        }
        pulls += 1;
        if e.slain {
...
pub fn list_mobs(ing: &Ingest) -> Vec<MobDto> {
    ...
    // list requires the player to have actually landed damage in it (see
    // below), so an empty store correctly yields an empty list rather than
