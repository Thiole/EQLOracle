...
fn fixture() -> Option<Vec<u8>> {
    std::fs::read(concat!(
        ...
        "/../../fixtures/reference-slice.log"
    ))
    .ok()
}

fn engine() -> Engine {
    let src = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/../../packs/eql.toml"))
        .unwrap();
...
fn encounters_from_a_real_session() {
    ...
    let slain = t
        .done
        .iter()
        .filter(|e| e.end_reason == Some(EndReason::Slain))
        .count();
    let timed = t
        .done
        .iter()
        .filter(|e| e.end_reason == Some(EndReason::Timeout))
        .count();
    eprintln!("damage events {dmg_events}  deaths {deaths}");
    eprintln!(
        "encounters {} (slain {slain}, timeout {timed})",
        t.done.len()
    );
