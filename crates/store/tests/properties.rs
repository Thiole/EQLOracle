...
fn random_store(rng: &mut Rng) -> (Store, u64, usize) {
    ...
    let abils: Vec<_> = (0..rng.range(3, 20))
        ...
        .collect();
    let actors: Vec<_> = (0..rng.range(1, 8))
        .map(|i| s.sym(&format!("actor{i}")))
        .collect();
    ...
    for e in 0..rng.range(1, 30) {
        ...
        let enc = s.open_encounter(mob, ts, s.len() as u32, None);
        ...
        s.close_encounter(enc, ts, true, false);
...
fn ability_rows_partition_the_damage_exactly() {
    ...
    for round in 0..40 {
        ...
        assert_eq!(
            got, expect,
            "seed {sd} round {round}: damage lost or duplicated"
        );
...
fn encounter_ranges_partition_the_store() {
    ...
    for round in 0..40 {
        ...
        assert_eq!(
            sum, expect,
            "seed {sd} round {round}: encounter ranges do not cover the store"
        );
    }
}
...
fn tag_rollup_never_exceeds_the_ability_total() {
    ...
    for round in 0..30 {
        ...
        for (name, tot, _) in roll_up_by_tag(&rows) {
            assert!(
                tot <= expect,
                "seed {sd} round {round}: tag {name} exceeds total"
            );
        }
    }
}
...
fn eviction_preserves_surviving_encounter_totals() {
    ...
    for round in 0..30 {
        ...
        assert_eq!(
            before, after,
            "seed {sd} round {round}: eviction corrupted survivors"
        );
    }
}
...
fn window_sums_are_monotone_in_window_width() {
    ...
    for round in 0..20 {
        ...
        for w in [1_000i64, 5_000, 30_000, 300_000, 10_000_000] {
            ...
            assert!(
                t >= prev,
                "seed {sd} round {round}: widening the window lost damage"
            );
            prev = t;
        }
    }
